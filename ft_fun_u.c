...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/26 23:12:58 by cdanette          #+#    #+#             */
/*   Updated: 2021/01/27 00:28:15 by cdanette         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

char	*ft_uzero(char *str, int len, int kk)
{
	char	*new_str;
	int		i;
	char	*tmp;
...
char	*ft_uzero2(char *str, int len, int kk)
{
	char	*new_str;
	int		i;
	char	*tmp;
...
char	*ft_uspace(char *str, t_hole hole, int len)
{
	char	*new_str;
	int		i;
...
int		ft_fun_u(unsigned int n, t_hole hole)
{
	char	*number;
	int		len;
	...
	if (hole.prec == 0 && (ft_atoi(number) == 0 || hole.width > len))
		number = ft_uspace(number, hole, len);
	if (hole.prec > len)
		number = ft_uzero(number, len, hole.prec);
	...
	if (hole.width > len)
	{
		if (hole.prec == 0)
			...
			number = ft_uzero2(number, len, hole.width);
		else
			number = ft_uspace(number, hole, len);
	}
	len = ft_strlen(number);
